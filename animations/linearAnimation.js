...
LinerAnimation.prototype.init = function () {
...
LinerAnimation.prototype.initNextAnimation = function() {
  ...
  let subtraction_vector = vec3.subtract(subtraction_vector, this.p2, this.p1);
  ...
  let matrix_angle_y = mat4.fromRotation(mat4.create(), matrix_angle_y, unit_vector);
  let matrix_angle_z = mat4.fromRotation(mat4.create(), matrix_angle_z, unit_vector);
...
LinearAnimation.prototype.getMatrix = function() {
  ...
  if(this.current_animation >= control_points.length - 1)
    return null;
  let deltaT = performance.now() - this.initTime;
